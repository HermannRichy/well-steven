...
const WhyChooseUs: React.FC = () => {
  const features = [
    {
      icon: <BarChart3 className="w-5 h-5 md:w-6 md:h-6 text-slate-900" />,
      title: "L'orientation client",
      description: "Cela implique une écoute active, une amélioration continue basée sur les retours d'information et la création de valeur."
    },
    {
                ...
                <div key={index} className="space-y-6 lg:space-y-10">
                  <div className="flex items-start gap-4 lg:gap-6">
                    ...
                    <div className="space-y-2 lg:space-y-4">
                      <h3 className="text-lg lg:text-xl font-bold text-slate-900">{feature.title}</h3>
                      <p className="text-slate-500 leading-relaxed text-sm lg:text-base font-light max-w-md">
                        {feature.description}
                      </p>
                    </div>
                  </div>
                  {index === 0 && <div className="h-[1px] w-full bg-slate-200" />}
                </div>
              ))}
            </div>
